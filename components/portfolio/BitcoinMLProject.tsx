...
export default function BitcoinMLProject() {
  return (
    <ProjectDetail data={{
      ...
      year: '2025',
      tagline: 'Multi-model ensemble for Bitcoin price prediction using LSTM, XGBoost, and Random Forest. The LSTM reaches 12.19% MAPE on price regression, trained on 12 years of BTC-USD data with 18 engineered indicators.',
      overview: 'Predicting cryptocurrency prices is one of the hardest time-series problems: high noise, regime changes, and complex temporal dependencies. This project builds a three-model ensemble to attack the problem from different angles: LSTM for continuous price regression using 90-day sequences, XGBoost and Random Forest for directional classification using 18 technical indicators. The LSTM reaches 12.19% MAPE on held-out test data. The classifiers do not beat chance, and that is the honest result.',
      keyFeatures: [
      ...
      sections: [
        ...
        },
        {
          ...
          body: 'LSTM regression reaches 12.19% MAPE, MAE $2,847, RMSE $5,219. Read that with care: on a slow-moving price series a naive carry-forward also scores well on MAPE, so the number only means something next to that baseline. XGBoost and Random Forest classification sit near 48–49%, which is expected: directional prediction on daily crypto data is close to random due to market efficiency. The ensemble combines LSTM price estimates with XGBoost/RF direction signals for confidence-weighted position sizing.',
        },
        {
